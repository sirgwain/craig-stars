...
After all that, you'll also need to install 2 important Go dependencies: [Mage](https://github.com/magefile/mage) and [Air](https://github.com/air-verse/air). The former is a make-like build tool & command executer written in Go, while the latter is a server utility that aids with automatic backend server restarting.

Both can be installed using `go install`:
