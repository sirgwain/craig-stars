...
# always redo these
.PHONY: run images build bench test clean dev dev_backend dev_frontend
...
bench:
	go test run=^$ -bench=. -benchmem -benchtime=2s -benchcount=5 -outfile=tmp/go-benchresults.json
