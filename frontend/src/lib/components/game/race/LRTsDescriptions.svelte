<script lang="ts">
	...
	const getDescriptionForLRT = (lrt: LRT) => {
		switch (lrt) {
			...
			case LRT.UR:
				return 'When you scrap a fleet at a starbase, you will recover 90% of the minerals and 70% of the resources used to produce the fleet, avaliable for production in the next year. Scrapping at a planet gives you half the amount of minerals and resources.';
			case LRT.NRSE:
				return 'You will not be able to build the Radiating Hydro-Ram Scoop, Sub-Galactic Fuel Scoop, Trans-Galactic Fuel Scoop, Trans-Galactic Super Scoop, Trans-Galactic Mizer Scoop or the Galaxy Scoop. You will be able to build the Interspace-10 engine, which can travel at warp 10 without taking damage and has lower technology requirements.';
			case LRT.OBRM:
			...
			case LRT.BET:
				return 'New technologies initially cost twice as much to build. As soon as you exceed all of the tech requirements by one level, the cost drops back to normal. Miniaturization (the lowering of production costs for lower-tech items) occurs at 5% per level, up to 80%. (Without this trait, miniaturization occurs at 4% and caps at 75%.)';
			case LRT.RS:
				return 'All shields are 40% stronger than their listed rating. Shields regenerate 10% of their maximum strength after each round of battle until destroyed. However, all armor components will only have 50% of their rated strength.';
			case LRT.MA:
