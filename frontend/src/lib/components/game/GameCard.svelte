		...
		<div class="card-body">
			...
			<div class="flex flex-col">
				<div class="flex flex-row">
					...
					<div class:text-error={game.state === GameState.GENERATING_TURN_ERROR}>
						{enumToString(GameState, game.state)}
					</div>
				</div>
				<div class="flex flex-row">
					<div class="text-right font-semibold mr-2 w-32">Size</div>
					<div>{enumToString(Size, game.size)}</div>
				</div>
				<div class="flex flex-row">
					<div class="text-right font-semibold mr-2 w-32">Density</div>
					<div>{enumToString(Density, game.density)}</div>
				</div>
				<div class="flex flex-row">
