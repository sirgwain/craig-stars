<script lang="ts">
	...
	$: {
		if ($commandedPlanet && $myMapObjectsByPosition) {
			const mapObjectsByPosition = $myMapObjectsByPosition[positionKey($commandedPlanet)];
	...
	const transfer = () => {
		if (selectedFleet) {
			EventManager.publishCargoTransferDialogRequestedEvent(selectedFleet, $commandedPlanet);
		}
	};
