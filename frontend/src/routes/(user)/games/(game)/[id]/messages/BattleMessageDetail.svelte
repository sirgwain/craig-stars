...
{:else if details}
	{#if message.type === PlayerMessageBattle}
		...
		{:else if details.ourDead === 0 && details.theirDead === details.theirs}
			...
			{/if} by your fleet of {details.ours ?? 0}
			{details.ours === 1 ? 'ship' : 'ships'}, which did not suffer a single casualty.
		{:else if details.ourDead === details.ours && details.theirDead === 0}
			...
			{/if}
			by the enemy's {details.theirs ?? 0}
	...
	{:else if message.type === PlayerMessageBattleAlly}
		...
		{:else if details.ourDead === 0 && details.theirDead === details.theirs}
			...
			{:else}
				A single enemy ship was destroyed
			{/if} by their fleet of {details.ours ?? 0}
			{details.ours === 1 ? 'ship' : 'ships'}, which did not suffer a single casualty.
		{:else if details.ourDead === details.ours && details.theirDead === 0}
			{#if details.ours > 1}
