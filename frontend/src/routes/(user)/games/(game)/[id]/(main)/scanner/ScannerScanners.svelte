<script lang="ts">
	...
	const scanners = derived([universe, player, settings], ([universe, player, settings]) => {
	...
	let scannerScale = derived([settings], ([s]) => s.scannerPercent / 100.0);
</script>

{#if $scanners && $settings.showScanners}
