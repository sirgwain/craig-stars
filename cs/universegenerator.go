	...
	for _, player := range ug.Players {
		...
		for _, startingPlanet := range player.Race.Spec.StartingPlanets {
			for _, startingFleet := range startingPlanet.StartingFleets {
				...
				design, err := DesignShip(&ug.Game.Rules, player, ug.log, hull, startingFleet.Name, num, int(startingFleet.HullSetNumber), startingFleet.Purpose, FleetPurposeFromShipDesignPurpose(startingFleet.Purpose))
				if err != nil {
