...
func (b *battle) fireBeamWeapon(weapon *battleWeaponSlot, targets []*battleToken) {
	...
	for _, target := range targets {
		...
		b.log.Debug().Msgf("%v fired a %vx%d at %v (shields: %v, armor: %v, beamDefense: %f%%, %v@%v damage) for %v damage", weapon.token, weapon.slot.HullComponent, weapon.slotQuantity*weapon.token.Quantity, target, target.totalStackShields, target.armor, target.beamDefense, target.Quantity, target.Damage, bwd.armorDamage)
