...
func GaussianPdf(mean, variance, x float64) float64 {
...
func NormalizedGaussianPdf(mean, variance float64, x int) float64 {
...
func NormalSample(random rng, mean, variance float64, max int) int {
	x := random.Intn(max)
	y := random.Float64()
