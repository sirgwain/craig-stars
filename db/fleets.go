...
type Fleet struct {
	...
	ImmediateCargoTransfers *ImmediateCargoTransfers `json:"immediateCargoTransfers"`
...
type ImmediateCargoTransfers []cs.ImmediateCargoTransfer
...
// db serializer to serialize this to JSON
func (item *ImmediateCargoTransfers) Value() (driver.Value, error) {
	if item == nil {
		return nil, nil
	}
...
// db deserializer to read this from JSON
func (item *ImmediateCargoTransfers) Scan(src interface{}) error {
	return scanJSON(src, item)
}
...
// TODO: move this logic into a wrapInTransaction instead of in the db
