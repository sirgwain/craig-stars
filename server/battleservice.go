...
// NewBattleServiceHandler constructs the BattleService handler.
func NewBattleServiceHandler() craig_starsv1connect.BattleServiceHandler {
...
func (s *battleService) GetTestBattle(ctx context.Context, req *connect.Request[craig_starsv1.GetTestBattleRequest]) (*connect.Response[craig_starsv1.GetTestBattleResponse], error) {
	...
	player1.Intels.PlayerIntels = []cs.PlayerIntel{{Num: player1.Num}, {Num: player2.Num}}
	player2.Intels.PlayerIntels = []cs.PlayerIntel{{Num: player1.Num}, {Num: player2.Num}}

	player1.Designs = append(player1.Designs,
		cs.NewShipDesign(player1.Num, 1).
		...
		cs.NewShipDesign(player1.Num, 2).
		...
		cs.NewShipDesign(player1.Num, 3).
		...
		cs.NewShipDesign(player1.Num, 4).
		...
		cs.NewShipDesign(player1.Num, 5).
			WithName("Frigate").
		...
		cs.NewShipDesign(player1.Num, 6).
		...
		cs.NewShipDesign(player1.Num, 7).
	...
	player2.Designs = append(player2.Designs,
		cs.NewShipDesign(player2.Num, 1).
			WithName("Teamster").
		...
		cs.NewShipDesign(player2.Num, 2).
			WithName("Long Range Scout").
			WithHull(cs.Scout.Name).
			WithSlots([]cs.ShipDesignSlot{
				{HullComponent: cs.LongHump6.Name, HullSlotIndex: 1, Quantity: 1},
		...
		cs.NewShipDesign(player2.Num, 3).
			WithName("Stalwart Defender").
			WithHull(cs.Destroyer.Name).
			WithSlots([]cs.ShipDesignSlot{
				{HullComponent: cs.LongHump6.Name, HullSlotIndex: 1, Quantity: 1},
		...
		cs.NewShipDesign(player2.Num, 4).
		...
		cs.NewShipDesign(player2.Num, 5).
			WithName("FF 8").
			WithHull(cs.Frigate.Name).
			WithSlots([]cs.ShipDesignSlot{
				{HullComponent: cs.FuelMizer.Name, HullSlotIndex: 1, Quantity: 1},
		...
		cs.NewShipDesign(player2.Num, 6).
			WithName("DD S 10").
			WithHull(cs.Destroyer.Name).
			WithSlots([]cs.ShipDesignSlot{
				{HullComponent: cs.FuelMizer.Name, HullSlotIndex: 1, Quantity: 1},
		...
		cs.NewShipDesign(player2.Num, 7).
			WithName("Empty Spacedock").
			WithHull(cs.SpaceDock.Name).
			WithSlots([]cs.ShipDesignSlot{}),
		cs.NewShipDesign(player2.Num, 8).
			WithName("Laser/Torpdo Space Station").
			WithHull(cs.SpaceStation.Name).
			WithSlots([]cs.ShipDesignSlot{
				{HullComponent: cs.ColloidalPhaser.Name, HullSlotIndex: 2, Quantity: 8},
				{HullComponent: cs.MoleSkinShield.Name, HullSlotIndex: 3, Quantity: 8},
				{HullComponent: cs.DeltaTorpedo.Name, HullSlotIndex: 4, Quantity: 8},
				{HullComponent: cs.MoleSkinShield.Name, HullSlotIndex: 6, Quantity: 8},
				{HullComponent: cs.ColloidalPhaser.Name, HullSlotIndex: 8, Quantity: 8},
				{HullComponent: cs.ColloidalPhaser.Name, HullSlotIndex: 10, Quantity: 8},
			}),
	)
	...
	p2ArmedStation := player2.Designs[7]
	_ = p1BattleCruiser
	_ = p1Scout
	...
	if err != nil {
		return nil, connect.NewError(connect.CodeInternal, fmt.Errorf("failed to run battle: %w", err))
	}

	return connect.NewResponse(&craig_starsv1.GetTestBattleResponse{
		Player:  converter.C.ConvertCSPlayer(player1),
		Designs: converter.C.ConvertCSShipDesigns(player1.Designs),
		Intels:  converter.C.ConvertCSIntels(player1.Intels),
		Fleets:  converter.C.ConvertCSFleets(fleets),
		Battle:  converter.C.ConvertCSBattleRecord(*record),
	}), nil
}
